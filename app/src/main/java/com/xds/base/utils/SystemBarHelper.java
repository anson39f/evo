...
public class SystemBarHelper {
    ...
    public static void setStatusBarDarkMode(Activity activity, boolean isFontColorDark) {

        setStatusBarDarkMode(activity.getWindow(), isFontColorDark);
    }
     ...
     * 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)
     */
    public static void setStatusBarDarkMode(Window window, boolean isFontColorDark) {

        if (isFlyme4Later()) {
            setStatusBarDarkModeForFlyme4(window, isFontColorDark);
        } else if (isMIUI6Later()) {
            setStatusBarDarkModeForMIUI6(window, isFontColorDark);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //            setStatusBarDarkModeForM(window);
            setStatusBarLightMode(window, isFontColorDark);
        }
    }
...
//                        window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
     ...
     * 创建假的状态栏View
     */
    public static void setStatusBar(ViewGroup container,
