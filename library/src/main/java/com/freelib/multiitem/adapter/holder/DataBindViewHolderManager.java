...
public class DataBindViewHolderManager<T> extends BindViewHolderManager<T> {
     ...
     */
    public DataBindViewHolderManager(@LayoutRes int itemLayoutId,final int bindVariableId) {
        this(itemLayoutId, new ItemBindView<T>() {
            @Override
            public void onBindViewHolder(ViewDataBinding dataBinding, T data) {
                dataBinding.setVariable(bindVariableId, data);
            }
        });
    }
